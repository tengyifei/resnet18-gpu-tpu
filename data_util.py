...
def download_dataset(split="test"):
  ...
  train_dataset = load_dataset(
    "flwrlabs/celeba", split=split, trust_remote_code=True, cache_dir="cache"
  )
  ...
  def transform_example(ex):
    ...
    return {
      "image": [img for img, label in img_bbox_pairs],
...
def download_dataset_custom_split(seed=42):
  ...
  train_dataset = CelebALazyDataset(
    combined_dataset, train_indices, unique_labels, transform
  )

  print("Creating test dataset")
  test_dataset = CelebALazyDataset(
    combined_dataset, test_indices, unique_labels, transform
  )
...
def download_dataset_custom_split_cached(seed=42):
  ...
  if (
    (train_path / ".done").exists()
    and (test_path / ".done").exists()
    ...
    try:
      ...
      if metadata.get("seed") == seed:
        ...
        print(
          f"Warning: Cached dataset has seed {metadata.get('seed')}, but requested seed is {seed}"
        )
  ...
  if cache_is_valid:
    ...
    # Create datasets
    train_dataset = CustomCachedDataset(train_path)
    test_dataset = CustomCachedDataset(test_path)
  ...
  # Create new cache
    ...
    # Save metadata
    metadata = {"num_classes": NUM_CLASSES, "seed": seed}
...
class CustomCachedDataset(torch.utils.data.Dataset):
